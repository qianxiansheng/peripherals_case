...
static SADC_ftCal_t SADC_ftCal;
...
{
    if (SADC_ftCal.f)
        return SADC_ftCal.f(data & ADC_MK_MASK(14));
...
static uint32_t ADC_FtCal(uint32_t data)
{
	if (data >= 8192) {
		return (data - 8192) * 200000 / (SADC_ftCal.k * ADC_PgaGainGet()) + 18192 - SADC_ftCal.Coseq;
	} else {
		return (-1) * ((SADC_ftCal.Coseq - data - 1808) * 200000 / (SADC_ftCal.k * ADC_PgaGainGet()) - 8192);
	}
}

void ADC_FtCaliSingle(uint32_t V1, uint32_t V2, uint32_t Vp, uint32_t Cout1, uint32_t Cout2)
{
    uint32_t Cin1, Cin2;
    Cin1 = V1 * 16384 / Vp;
    Cin2 = V2 * 16384 / Vp;
    uint32_t k, Coseq;
    k = (Cout2 - Cout1) * 100000 / (1 * (Cin2 - Cin1));
    Coseq = 18192 - (Cin1 * (Cout2 - 8192) + Cin2 * (8192 - Cout1)) / (Cout2 - Cout1);
    SADC_ftCal.k = k;
    SADC_ftCal.Coseq = Coseq;
    SADC_ftCal.f = ADC_FtCal;
    ADC_adcCal.vref_P = Vp / 1000;
}
...
void ADC_VrefCalibration(void)
{
    ...
    ADC_ConvCfg(CONTINUES_MODE, PGA_GAIN_2, 1, ADC_CH_9, 15, 0, 
        SINGLE_END_MODE, SYSCTRL_GetClk(SYSCTRL_ITEM_APB_ADC) / 900000);
    ADC_Start(1);
